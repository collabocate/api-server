...
export const restoreOneTrashService = async (user_id: string, paramsId: string) => {
  ...
  if (!Model) {
    notFoundErr(`No record found for collection: '${trash.collectionName}' to be restored`)
  }

  // Restore the document to the original collection
  await Model.create(trash.trashedDocument);
...
export const getOneSpecificTrashService = async (user_id: string, paramsId: string, collection_name: string) => {
  ...
  if(!query){
    notFoundErr(`No record found for provided ${collection_name} with ID: ${paramsId}`);
  }
  return query;
}